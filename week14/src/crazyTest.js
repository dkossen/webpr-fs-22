...
crazySuite.add("equals", assert => {
    ...
    assert.is( {} == NaN, false);
    assert.is( true == 1, true);
    assert.is( "" == false, true);
    assert.is( 0 == false, true);
    assert.is( [] == false, true);
    assert.is( "0" == false, true);
...
crazySuite.add("other", assert => {
    ...
    assert.is( {} + [],      '[object Object]'); // try in console -> 0 /--> ({} + []) -> "[object Object]"
...
crazySuite.add("compare", assert => {
    ...
    assert.is( {} == {} ,            false);
    assert.is( {} >  {} ,            false);
    assert.is( {} >= {} ,            true); // "not smaller than"
